...
class LSTMAE(nn.Module):
    def __init__(self, input_size, hidden_size, latent_dim, num_layers, dropout_ratio=0): #, seq_len, use_act=False):
        ...
        self.encoder = Encoder(input_size=input_size, hidden_size=hidden_size, dropout=dropout_ratio, num_layers=num_layers) # , seq_len=seq_len)
        self.encoder_fc = nn.Linear(hidden_size, latent_dim)
        self.decoder_fc = nn.Linear(latent_dim, hidden_size)
        self.decoder = Decoder(input_size=input_size, hidden_size=hidden_size, dropout=dropout_ratio, num_layers=num_layers) # , seq_len=seq_len, use_act=use_act)
