...
# Funktion zur Erstellung eines DataFrames mit korrekter Chicken-Nummerierung
def create_dataframe(data, condition, method, start_i):
    df = pd.DataFrame(data)
    df['Chicken'] = [f'Chicken{i+start_i}' for i in range(len(data))]
...
data_eyecounting = pd.concat([
    create_dataframe(central_control_data_1, 'Central Control', 'EyeCounting', start_i=2),
    create_dataframe(peripheral_control_data_1, 'Peripheral Control', 'EyeCounting', start_i=2),
    create_dataframe(central_treatment_data_1, 'Central Treatment', 'EyeCounting', start_i=2),
    create_dataframe(peripheral_treatment_data_1, 'Peripheral Treatment', 'EyeCounting', start_i=2),
    create_dataframe(central_treatment2_data_1, 'Central Treatment 2', 'EyeCounting', start_i=5),  # Start bei Chicken05
    create_dataframe(peripheral_treatment2_data_1, 'Peripheral Treatment 2', 'EyeCounting', start_i=5)  # Start bei Chicken05
])

data_imagej = pd.concat([
    create_dataframe(central_control_data_2, 'Central Control', 'ImageJ', start_i=2),
    create_dataframe(peripheral_control_data_2, 'Peripheral Control', 'ImageJ', start_i=2),
    create_dataframe(central_treatment_data_2, 'Central Treatment', 'ImageJ', start_i=2),
    create_dataframe(peripheral_treatment_data_2, 'Peripheral Treatment', 'ImageJ', start_i=2),
    create_dataframe(central_treatment2_data_2, 'Central Treatment 2', 'ImageJ', start_i=5),  # Start bei Chicken05
    create_dataframe(peripheral_treatment2_data_2, 'Peripheral Treatment 2', 'ImageJ', start_i=5)  # Start bei Chicken05
])

data_python = pd.concat([
    create_dataframe(central_control_data_3, 'Central Control', 'Python', start_i=2),
    create_dataframe(peripheral_control_data_3, 'Peripheral Control', 'Python', start_i=2),
    create_dataframe(central_treatment_data_3, 'Central Treatment', 'Python', start_i=2),
    create_dataframe(peripheral_treatment_data_3, 'Peripheral Treatment', 'Python', start_i=2),
    create_dataframe(central_treatment2_data_3, 'Central Treatment 2', 'Python', start_i=5),  # Start bei Chicken05
    create_dataframe(peripheral_treatment2_data_3, 'Peripheral Treatment 2', 'Python', start_i=5)  # Start bei Chicken05
])

# Alle Daten kombinieren
all_data = pd.concat([data_eyecounting, data_imagej, data_python])

# Erstellen einer gemeinsamen Spalte aus Condition und Chicken
all_data['Condition_Chicken'] = all_data['Condition'] + ' - ' + all_data['Chicken']

# Option: Die 'Condition' und 'Chicken' Spalten können entfernt werden, wenn sie nicht mehr benötigt werden
all_data = all_data.drop(columns=['Condition', 'Chicken'])
...
# Statt 'Chicken', nutze die neue kombinierte Spalte 'Condition_Chicken' als Subject
rm_anova = pg.rm_anova(data=all_data, dv='Value', within=['Method'], subject='Condition_Chicken', detailed=True)
...
# Perform pairwise comparisons across methods
pairwise_results = pg.pairwise_tests(data=all_data, dv='Value', between='Method')

# Rename the columns for better readability
pairwise_results.rename(columns={
    'A': 'Method A',
    'B': 'Method B',
    'T': 'T-Value',
    'p-val': 'p-value',
    'p-unc': 'p-uncorrected p-value',
    'cohen-d': 'Cohen\'s d'
}, inplace=True)

# Round results to 4 decimal places
pairwise_results = pairwise_results.round(4)


# Zähle die Anzahl der Werte (Zeilen) für jede Methode
method_counts = all_data.groupby('Method').size()

# Ausgabe der Anzahl der Werte für jede Methode
print(method_counts)

# Berechne die Anzahl der Werte für jede Kombination aus Methode und Bedingung
count_condition_method_values = all_data.groupby(['Method', 'Condition_Chicken'])['Value'].count()

# Ausgabe der Anzahl der Werte für jede Kombination aus Methode und Bedingung
print(count_condition_method_values)

# Save pairwise comparison results as an image
save_as_image(pairwise_results, "Pairwise Comparisons (Methods)", "pairwise_comparisons_results.png")
